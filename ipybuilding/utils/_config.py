...
def _get_home_dir():
    ...
    home_dir = os.path.join(user_dir, ".ipybuilding")
...
def _get_config_path():
    ...
    config_path = os.path.join(home_dir, "ipybuilding.json")
    return config_path


def _get_data_path():
    """Get ipybuilding data directory."""
    user_dir = _get_user_dir()
    data_dir = os.path.join(user_dir, "ipybuilding_data")
...
def _save_config(config):
    """Save ipybuilding config."""
    with open(_get_config_path(), "w", encoding="utf-8") as f:
        json.dump(config, f)


def get_config():
    """Read preferences from ipybuilding' config file.
    Returns
    -------
    config : dict
